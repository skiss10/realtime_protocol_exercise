...
# define load_balancer interface with memory store
SESSION_STORAGE = InMemoryStore()

# # List of server addresses
# servers = {'localhost:12331': None, 'localhost:12332': None}

# def server_alive(connection):
#     pass
...
#     """
#     hash_ring = {}
#     for address, connection in servers.items():
#         if server_alive(connection):
#             hash_value = hashlib.sha256(address.encode('utf-8')).hexdigest()
#             hash_ring[hash_value] = address
#     return hash_ring

# def choose_server(request):
#     """
#     Choose a server for the given request
#     """
#     hash_ring = create_hash_ring()
#     hash_value = hashlib.sha256(request.encode('utf-8')).hexdigest()
#     sorted_hash_values = sorted(hash_ring.keys())
#     for hash_key in sorted_hash_values:
#         if hash_key >= hash_value:
#             return hash_ring[hash_key]
#     return hash_ring[sorted_hash_values[0]]

# # Example usage:
# server_address = choose_server('some request data')
# print('Chose server:', server_address)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_server_socket:
        ...
        try:
            ...
            while True:
                ...
                # store server info locally
                SESSION_STORAGE.set(server_connection.id, server_connection)
                ...
                # # start thread to route messages from client socket to server socket
                # message_router_thread = threading.Thread(target=server_pulse, args=(server_connection,))
                # message_router_thread.start()
